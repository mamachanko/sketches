...
export default () => new p5base((p5) => {
    ...
    const cols = parseInt(width * .5);
    const rows = parseInt(height * .05);
    ...
    p5.draw = () => {
        ...
        const radius = p5.min(height, width) / 2;
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                if (p5.dist(cellCenterX, cellCenterY, circleCenterX, circleCenterY) > radius) {
                    p5.line(
